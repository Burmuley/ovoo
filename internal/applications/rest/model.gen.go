...
type DomainData struct {
	...
	// Owner Present only for personal domains
	Owner *UserData `json:"owner,omitempty"`

	// Type Whether domain is system-wide or owned by the current user
...
type ProtectedAddressData struct {
	...
	// Verified Indicates whether the Protected Address is verified; also influences the `active` state - not verified Protected Address are not active
	Verified bool `json:"verified"`

	// VerifiedAt Timestamp when email verification has been done
	VerifiedAt *time.Time `json:"verified_at,omitempty"`
}
...
// VerifyProtectedAddressResponse defines model for verifyProtectedAddressResponse.
type VerifyProtectedAddressResponse = ProtectedAddressData
...
type CreateDomainRequest struct {
	...
	// Type Domain scope. Admin users may pass `global` to create a system-wide domain. Defaults to `personal` if omitted. Non-admin users receive 403 if `global` is specified.
	Type DomainType `json:"type"`

	// VerificationType Domain verification type: DNS TXT or CNAME records are currently supported.
...
// ValidatePrAddrTokenRequest defines model for validatePrAddrTokenRequest.
type ValidatePrAddrTokenRequest struct {
	Token string `json:"token"`
}
...
// ValidatePrAddrTokenJSONBody defines parameters for ValidatePrAddrToken.
type ValidatePrAddrTokenJSONBody struct {
	Token string `json:"token"`
}
...
// ValidatePrAddrTokenJSONRequestBody defines body for ValidatePrAddrToken for application/json ContentType.
type ValidatePrAddrTokenJSONRequestBody ValidatePrAddrTokenJSONBody
