<template>
    <CCard style="max-width: 540px;">
        <CCardHeader class="fw-semibold d-flex justify-content-between align-items-center">
            <span>{{ currentStep === 1 ? 'Add New Domain' : 'Verify Your Domain' }}</span>
            <small class="text-body-secondary fw-normal">Step {{ currentStep }} of 2</small>
        </CCardHeader>
        <CCardBody>
            ...
            <CForm v-if="currentStep === 1" @submit.prevent="createDomain">
                ...
                <CAlert v-if="errorMessages.length" color="danger" class="mt-3">
