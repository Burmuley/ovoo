                ...
                <template v-else>
                    <CTableRow v-for="domain in domains" :key="domain.id">
                        ...
                        <CTableDataCell class="text-center text-nowrap">
                            ...
                            <CBadge v-if="domain.type === 'personal'" :color="domain.verified ? 'success' : 'warning'"
                                class="ms-1">
...
<CModal :visible="showAddModal" @close="onFormDone" size="lg">
    <CModalHeader>
        <CModalTitle>Add New Domain</CModalTitle>
    </CModalHeader>
    <CModalBody>
        <AddDomainForm v-if="showAddModal" :user-info="props.userInfo" @done="onFormDone" />
    </CModalBody>
</CModal>
</template>
